...
pickle_name_for_results = "simulation_results_CorInfoMax_CIFAR10_HiddenLayerSize_Ablation.pkl"
...
lr_start_list = [{'ff' : np.array([0.08, 0.04]), 'fb': np.array([np.nan, 0.04])}, 
                 {'ff' : np.array([0.07, 0.03]), 'fb': np.array([np.nan, 0.05])}]
...
setting_number = 0
for lambda_, lr_start, lr_decay_multiplier, neural_lr_start, neural_lr_rule, neural_dynamic_iterations_free, hopfield_g, use_three_phase, hidden_layer_size in product(lambda_list, lr_start_list, lr_decay_multiplier_list, neural_lr_start_list, neural_lr_rule_list, neural_dynamic_iterations_free_list, hopfield_g_list, use_three_phase_list, hidden_layer_size_list):
    setting_number += 1
    
    architecture = [int(32*32*3), hidden_layer_size, 10]
