...
pickle_name_for_results = "simulation_results_CorInfoMax_CIFAR10_4Layers_V1.pkl"
...
lr_start_list = [{'ff': np.array([0.25, 0.14, 0.075, 0.045]), 'fb': np.array([ np.nan, 0.095, 0.075, 0.04])},
                 {'ff': np.array([0.28, 0.18, 0.095, 0.085]), 'fb': np.array([ np.nan, 0.105, 0.085, 0.055])},
                 {'ff': np.array([0.2, 0.12, 0.095, 0.055]), 'fb': np.array([ np.nan, 0.085, 0.065, 0.035])}]
lr_decay_multiplier_list = [0.95]
neural_lr_start_list = [0.03, 0.05, 0.07]
neural_lr_stop = 0.001
neural_lr_rule_list = ["constant"]
neural_lr_decay_multiplier = 0.01
neural_dynamic_iterations_nudged = 15
neural_dynamic_iterations_free_list = [60, 80, 100]
hopfield_g_list = [0.1]
use_random_sign_beta = True
use_three_phase_list = [False,True]

n_epochs = 50
seed_list = [10*j for j in range(2)]

setting_number = 0
for lambda_, lr_start, lr_decay_multiplier, neural_lr_start, neural_lr_rule, neural_dynamic_iterations_free, hopfield_g, use_three_phase, beta in product(lambda_list, lr_start_list, lr_decay_multiplier_list, neural_lr_start_list, neural_lr_rule_list, neural_dynamic_iterations_free_list, hopfield_g_list, use_three_phase_list, beta_list):
    setting_number += 1
    hyperparams_dict = {"lr_start" : lr_start, "lr_decay_multiplier" : lr_decay_multiplier,
                        ...
                        "epsilon" : epsilon, "lambda" : lambda_, "beta" : beta,
