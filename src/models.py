...
class CorInfoMaxErrorProp():
    ...
    def calculate_neural_dynamics_grad(self, x, y, neurons, beta, mode = "train"):
        ...
        for jj in reversed(range(1, len(Wfb))):
            activated = self.activation_func(backward_errors[-1], self.activation_type)
            backward_errors.append(neurons[jj - 1] - (Wfb[jj]['weight'] @ activated[0] + Wfb[jj]['bias']))
    ...
    def batch_step(self, x, y, lr, neural_lr_start, neural_lr_stop, neural_lr_rule = "constant", 
        ...
        if optimizer == "sgd":
            ...
            for jj in range(1, len(Wfb)):
                Wfb[jj]['weight'] += lr['fb'] * torch.mean(outer_prod_broadcasting(backward_errors[jj - 1].T, forward_errors[jj].T), axis = 0)
                Wfb[jj]['bias'] += lr['fb'] * torch.mean(backward_errors[jj - 1], axis = 1, keepdims = True)
            ...
            for jj in range(1, len(Wfb)):
                grad_Wfb_weight = torch.mean(outer_prod_broadcasting(backward_errors[jj - 1].T, forward_errors[jj].T), axis = 0)
